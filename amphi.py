...
def mse(y_pred, y, give_rmse=False, repr=False):
    ...
    if not give_rmse and not repr:
        ...
        return "The Mean Squared Error is: {mse}"
    elif give_rmse and not repr:
        return mse, rmse

    return "The Mean Squared Error is: {mse}, The Root Mean Squared Error is: {rmse}"
...
class LinearRegerssion:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iters):
            ...
            dw = (1/m)*(np.dot(X.T, (y_hat - y))) # differentiation of the sq. error loss wrt weight
            db = (1/m)*(np.sum(y_hat - y)) # differentiation of the sq. error loss wrt bias
...
class Kmeans:
    ...
    def predict(self, X):
        ...
        random_sample_idxs = np.random.choice(self.n_samples, self.K, replace=False)  # vector of size K, default 3*1
        self.centroids = [self.X[idx] for idx in random_sample_idxs] # K*1 vector 
    ...
    def _closest_centroid(self, num, centroids):
        distances = [euclidian_distance(num, centroid) for centroid in centroids] # list of size K
...
class GMM:
    ...
    def start_EM(self):
        ...
        for step in range(self.n_iters+1):
            ...
            for j in range(self.k):
                likelihood.append(stats.multivariate_normal.pdf(x=self.X, mean=self.means[j], cov=self.covs[j]))
    ...
    def plot(self):
        ...
        x = np.linspace(np.min(self.X[..., 0])-1, np.max(self.X[..., 0]+1), 100)
        ...
        likelihood = []
        for j in range(self.k):
            likelihood.append(stats.multivariate_normal.pdf(x=pos, mean=self.means[j], cov=self.covs[j]))
        ...
        for c in range(self.k):
            ...
            plt.scatter(pos[pred_ids[0], 0], pos[pred_ids[0], 1], color = colors[c], alpha=0.2, marker='s', edgecolors='none')
        
        plt.scatter(self.X[:, 0], self.X[:, 1], edgecolors='grey', facecolors='none')
